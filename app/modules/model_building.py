...
class TopicModelling:
    ...
    def build_model(self):
        ...
        The `build_model` method builds an LDA model using the parameters specified during initialization. If the `tfidf_transform` parameter is set to True, the method applies the TF-IDF transformation
        * to the corpus before building the model. Otherwise, it uses the corpus as is.

        The LDA model is built using the `LdaModel` class from the Gensim library. The `corpus_used` parameter is passed as the training corpus, `id2word` specifies the dictionary mapping of
        * words to their numeric ids, `num_topics` determines the number of topics to be identified, `random_state` sets the random seed for reproducibility, `chunksize` specifies the number
        * of documents processed together, `passes` determines the number of times the model is trained on the entire corpus, and `per_word_topics` controls whether probabilities of words in
        * topics are returned.
